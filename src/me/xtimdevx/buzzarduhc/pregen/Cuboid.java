...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public Cuboid(Plugin instance, World world, int x1, int y1, int z1, int x2, int y2, int z2)
    {
        plugin = instance;
    ...
    public List<Chunk> getChunks()
    {
        ...
        PregenClass.max = Integer.valueOf(max);
        for (int x = x1; x <= x2; x += 16) {
            for (int z = z1; z <= z2; z += 16)
            {
                res.add(w.getChunkAt(x >> 4, z >> 4));
                for(Player online : Bukkit.getOnlinePlayers()) {
                    GameUtils.sendAction(online, "§fCalculating chunks... §8(§b" + current + "§8/§b" + max + "§8)");
                }
                current++;
            }
        }
        PregenClass.max = Integer.valueOf(current);
        return res;
    }
    ...
    public void generateChunks()
    {
        ...
        synchronized (this.thread)
        {
            ...
            for (int x = x1; x <= x2; x += 16) {
                for (int z = z1; z <= z2; z += 16)
                {
                    if ((!PregenClass.running.booleanValue()) || (this.cancel.booleanValue()))
                    {
                        this.thread.stop();
                        return;
                    }
                    ...
                    for(Player online : Bukkit.getOnlinePlayers()) {
                        GameUtils.sendAction(online,"Generating chunks... §8(§b" + current + "§8/§b" + max + "§8) (§b" + Math.round(current * 100.0F / (max / 1.0F) * 10.0F) / 10.0F + "%§8)");
                    }
                    label485:
                    current++;
                    if (Math.round((float)(Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory()) * 100.0F / ((float)Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F) / 10.0F > 80.0F)
                    {
                        while (Math.round((float)(Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory()) * 100.0F / ((float)Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F) / 10.0F > 60.0F) {
                            try
                            {
                                PregenClass.log("Memory usage too high, waiting... §8(§b" + Math.round(current * 100.0F / (max / 1.0F) * 10.0F) / 10.0F + "%§8)");
                                System.gc();
                                Thread.sleep(5000L);
                            }
                            catch (InterruptedException e)
